<template>
  <md-table v-model="NhacNoChuaThanhToanComputed" :table-header-color="tableHeaderColor" md-fixed-header>
    <md-table-row slot="md-table-row" slot-scope="{ item }">
      ...
      <md-table-cell md-label="STK gửi">{{ item.SoTaiKhoanNguoiGui }}</md-table-cell>
      ...
      <md-table-cell md-label="Hành động">
        <md-button style="height: 20px;" class="md-just-icon md-simple md-primary nofocus" @click="ThanhToanNhacNo">
          <md-icon>how_to_vote</md-icon>
          <md-tooltip md-direction="top">Thanh toán nhắc nợ</md-tooltip>
        </md-button>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState(['NhacNoChuaThanhToan']),
      NhacNoChuaThanhToanComputed(){
        let arr = JSON.parse(JSON.stringify(this.NhacNoChuaThanhToan));
        for (let i in arr){
    ...
    methods: {
      ThanhToanNhacNo(event){
        ...
        this.$emit('ThanhToanNhacNo',payload);
      },
      HuyNhacNo(event){
        let element = event.target.closest('tr').getElementsByTagName('td');
        let payload = {
        ...
        this.$emit('HuyNhacNoChuaThanhToan',payload);
      }
    }
  };
</script>
